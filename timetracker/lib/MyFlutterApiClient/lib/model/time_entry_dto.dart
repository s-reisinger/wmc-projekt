...
class TimeEntryDto {
  ...
  @override
  String toString() => 'TimeEntryDto[id=$id, employeeId=$employeeId, start=$start, end=$end, comment=$comment]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "TimeEntryDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "TimeEntryDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
